...
1. `fs(search, path="/wiki", query="<terms>")` (3-5 paraphrased queries) and `fs(read, path="/wiki/<path>")` the affected pages — don't trust titles, open them.
...
4. Apply with `fs(write, path="/wiki/<path>", content=<updated>)` (hashline `edit` for surgical changes) or `fs(mv, path="/wiki/<from>", to="/wiki/<to>")` (which rewrites inbound links). Check what links to a page before moving or archiving it.
5. Never hard-delete — `fs(rm, path="/wiki/<path>")` moves a page to the archive folder.
