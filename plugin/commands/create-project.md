...
1. `fs(search, path="/wiki", query="<terms>")` for relevant knowledge. [G1]
2. `fs(search, path="/skills", query="<topic>")` for procedures that should be attached. [G2/G3]

Then:
- Create the project — a project is a folder: `fs(mkdir, path="/projects/<name>")`. It starts with **no layers**; you define them by writing into them.
- `fs(write, path="/projects/<name>/<layer>/<lane>/brief.md", content=...)` a real brief at the earliest layer (goal, audience, constraints — 6-12 lines, not a placeholder). The layer + lane auto-create.
- Attach the relevant skills you found via the skill gates.
- `fs(ls, path="/projects/<name>")` to confirm the structure, and hand the user the `projectUrl` from the response.
