...
- **For a folder with NO connected repo, author skills in Drafted** with `fs(write, path="/skills/<slug>", content=..., readme=...)` (G2 gate first). A `README.md` (the `readme` param) is optional but recommended — it is what renders for the skill directory on GitHub if the skill is ever exported; omitting it creates the skill with a warning. A skill is a directory: `fs(write, path="/skills/<slug>/README.md", content=...)` and any other supporting file work once the skill exists, and `fs(ls, path="/skills/<slug>")` lists them. `fs(mv, path="/skills/<old>", to="/skills/<new>")` renames.
